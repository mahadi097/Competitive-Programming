...
ll solve2(ll val, ll amount, ll coin) {
    ...
    if(f && dp2[val][amount][coin] != -1) return dp2[val][amount][coin];
    if(!f && dp3[val][amount][coin] != -1) return dp3[val][amount][coin];
    ...
    if(f) return dp2[val][amount][coin] = t1;
